...
namespace Controllers\Adm\Site;
...
class Content extends \Controllers\Adm\AdmController {

    public function getPageIndex() {
        $totalPages = ContentData::isPage()->count();
        $pageSearch = ContentData::isPage();

        // Sorting and searching!
        $sortBy = in_array(Input::get("sort_by"), ["content_id", "title"]) ? Input::get("sort_by") : "content_id";
        $sortDir = in_array(Input::get("sort_dir"), ["ASC", "DESC"]) ? Input::get("sort_dir") : "ASC";

        // ORM it all!
        $pageSearch = $pageSearch->orderBy($sortBy, $sortDir)
                                 ->with("sections")
                                 ->with("children")
                                 ->paginate(50);

        return $this->viewMake("adm.site.content.page.index")
                    ->with("pages", $pageSearch)
                    ->with("sortBy", $sortBy)
